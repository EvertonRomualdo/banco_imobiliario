...
% get_current_player(+Players, +TurnIndex(1-based), -Player)
get_current_player(Players, TurnIndex, Player) :-
    nth1(TurnIndex, Players, Player).

% update_player_in_list(+Players, +NewPlayer, -NewPlayers)
% Substitui o jogador por id na mesma posição (não reordena a lista).
update_player_in_list([], _, []).
update_player_in_list([player(Id,_,_,_,_) | Rest], NewPlayer, [NewPlayer | Rest]) :-
    NewPlayer = player(Id,_,_,_,_), !.    % encontrado: substitui e mantém o resto
...
% next_turn_index(+Players, +CurrentTurnIndex, -NextTurnIndex)
